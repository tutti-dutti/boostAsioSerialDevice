...
export const FRIENDS: FriendDef[] = [
  ...
  { id: "seal", name: "Seal", emoji: "🦭", rarity: "rare", color: "#a8d0e8", damage: 50, range: 125, attackSpeed: 2.05, ability: "freeze", weaponRole: "antiSpeed", canEvolve: true, evolvesTo: "frostseal" },
  ...
  { id: "frostseal", name: "Frost Seal", emoji: "❄️", rarity: "mythical", color: "#c0e8ff", damage: 95, range: 170, attackSpeed: 2.25, ability: "freeze", weaponRole: "antiSpeed", evolvedForm: true },
...
export function pickFriend(lucky = false): FriendDef {
  ...
  if (lucky) {
    ...
    return pickFromRarityPool(summonable, rarity);
  }
...
/** Seal is a secret OP rare — slightly rarer within the rare pool so finding one feels special */
function pickFromRarityPool(summonable: FriendDef[], rarity: Rarity): FriendDef {
  const pool = summonable.filter((f) => f.rarity === rarity);
  if (rarity === "rare") {
    const seal = pool.find((f) => f.id === "seal");
    const others = pool.filter((f) => f.id !== "seal");
    // ~6% of rare summons are the secret seal (vs ~12% if uniform)
    if (seal && others.length && Math.random() < 0.06) return seal;
